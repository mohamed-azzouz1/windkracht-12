        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-gray-800">Niet Betaalde Lessen</h1>
                    <div class="flex space-x-2">
                        <a href="{{ route('admin.registrations.index') }}" class="bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-4 rounded">
                            <i class="fas fa-list mr-1"></i> Alle lessen
                        </a>
                ...
                <!-- Search & Filters -->
                <div class="mb-6">
                    <form action="{{ route('admin.registrations.unpaid') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                        ...
                        @if(request()->anyFilled(['search', 'date_from', 'date_to']))
                            <a href="{{ route('admin.registrations.unpaid') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium py-2 px-4 rounded">
                                Reset
                            </a>
                        @endif
                    </form>
                ...
                <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h3 class="font-semibold text-red-800">Openstaande betalingen</h3>
                            <p class="text-red-700">
                                {{ $registrations->total() }} klanten hebben nog niet betaald voor hun reservering.
                                @if($registrations->total() > 0)
                                    Totaal openstaand bedrag: €{{ number_format($registrations->sum(function($reg) { return $reg->package->price; }), 2, ',', '.') }}
                                @endif
                            </p>
                        </div>
                    </div>
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @if($registration->student->phone)
                                            <div class="text-xs text-gray-500">{{ $registration->student->phone }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-red-600">€{{ number_format($registration->package->price, 2, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-xs text-red-500 mt-1">
                                            <i class="fas fa-times-circle mr-1"></i> Niet betaald
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <form action="{{ route('admin.registrations.mark-as-paid', $registration->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PATCH')
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                        Alle klanten hebben betaald voor hun reserveringen. Goed gedaan!
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
