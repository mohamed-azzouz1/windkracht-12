            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <form action="{{ route('instructor.lessons.cancel.weather', $lesson->id) }}" method="POST">
                        @csrf
                        <button id="weatherCancelBtn" type="submit" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center">
                    ...
                    <form action="{{ route('instructor.lessons.cancel.sick', $lesson->id) }}" method="POST">
                        @csrf
                        <button id="sickCancelBtn" type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center">
                ...
                <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                    ...
                    <form id="customCancelForm" action="{{ route('instructor.lessons.cancel', $lesson->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="mt-6 flex items-center justify-end">
                            ...
                            <button type="submit" id="customCancelSubmitBtn" class="bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-lg">
                                Les Annuleren
                            </button>
                        </div>
                    </form>
...
<div id="confirmModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-lg shadow-lg p-6 max-w-md mx-auto">
        <h3 class="text-xl font-bold text-gray-900 mb-4">Bevestig annulering</h3>
        <p class="text-gray-700 mb-6">Weet je zeker dat je deze les wilt annuleren? De student zal hiervan op de hoogte worden gebracht.</p>
        <div class="flex justify-end space-x-4">
            <button id="cancelModalBtn" type="button" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">
                Terug
            </button>
            <button id="confirmModalBtn" type="button" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">
                Ja, annuleren
            </button>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const customCancelForm = document.getElementById('customCancelForm');
        const weatherCancelBtn = document.getElementById('weatherCancelBtn');
        const sickCancelBtn = document.getElementById('sickCancelBtn');
        const customCancelSubmitBtn = document.getElementById('customCancelSubmitBtn');
        ...
        if (weatherCancelBtn) {
            weatherCancelBtn.addEventListener('click', function(e) {
                showConfirmModal(e, weatherCancelBtn.closest('form'));
            });
        }
        
        if (sickCancelBtn) {
            sickCancelBtn.addEventListener('click', function(e) {
                showConfirmModal(e, sickCancelBtn.closest('form'));
            });
        }
        
        if (customCancelSubmitBtn) {
            customCancelSubmitBtn.addEventListener('click', function(e) {
                const reason = document.getElementById('cancellation_reason').value.trim();
                if (!reason) {
                    alert('Voer een reden in voor de annulering.');
                    return;
                }
                showConfirmModal(e, customCancelForm);
            });
        }
